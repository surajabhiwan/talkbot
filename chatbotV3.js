function createChatbot() {
  // Function to get the subscriptionId from the headers
  function getSubscriptionIdFromHeaders() {
    const headers = new Headers();
    // headers.append("Subscription-ID", "iamsun0001"); // Replace with dynamic fetching logic if needed

    // You can check if subscriptionId exists in headers
    return headers.get("Subscription-ID");
  }

  // Example usage of the subscriptionId
  const subscriptionId = getSubscriptionIdFromHeaders();
  ...
  // Create container for the chat0bot
  ...
  function sendSubscriptionIdToIframe() {
    ...
    if (iframe) {
      ...
      iframe.contentWindow.postMessage(
        { type: "authenticate", subscriptionId: subscriptionId },
        "*"
      );
    }
  }
